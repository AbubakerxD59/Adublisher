...
class Publisher_model extends CI_Model
{
	...
	public function get_pinterest_login_url()
	{
		...
		$login_url = $pinterest->auth->getLoginUrl($redirect_url, array('boards:read', 'pins:read', 'boards:write', 'pins:write', 'user_accounts:read'));
		return $login_url;
	}
	...
	public function get_user_ig_account_details($ig_account_id, $fb_page_id, $fb_page_name, $ig_access_token)
	{
		...
		if (isset($ig_account_info['id'])) {
			...
			$profile_pic = isset($user['picture']['data']['url']) ? saveImageFromUrl($user['picture']['data']['url'], $user_id, $user["id"]) : '';
			// get user details
			$data = [
				'user_id' => $user_id,
