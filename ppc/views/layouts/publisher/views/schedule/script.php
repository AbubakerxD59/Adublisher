            ...
            success: function(response) {
                if (response.status) {
                        ...
                        $.each(response.data.fbpages, function(index, value) {
                            ...
                            if (value.channel_active == 1) {
                                ...
                                $("#channels").append(
                                    ...
                                    value.id +
                                    "' style='border: 2px solid green;'><img style='width:25px;height:25px;' src='" + facebook_page_icon + "' class='rounded' alt='profile_pic'> " +
                                    value.page_name +
                    ...
                    if (response.data.ig_accounts.length > 0) {
                        ...
                        $.each(response.data.ig_accounts, function(index,
                            value) {
                            if (value.channel_active == 1) {
                                facebook_check = false;
                                $("#channels").append(
...
<script>
    ...
    $("#channels").on("mouseenter mouseleave", ".channel-button", function(event) {
        var button = $(this);
        var deleteButton = button.find(
            '.delete-button'); // Find the delete button inside the channel-button
        ...
        if (event.type === "mouseenter") {
            ...
            deleteButton.on("click", function(e) {
                e
                    .stopPropagation(); // Prevent the button click from triggering the button's click event
                var id = button.data("id");
                var pageType = button.data(
                    "type"); // Get the type of the clicked button
                ...
                if (pageType == 'facebook') {
                    ...
                    confirmationTextInside =
                        "Disconnect This Facebook Page Only |or| Also Delete It's Scheduled Posts?";
                    disconnectOnly = "Disconnect Facebook Page Only";
                    disconnectDeleteScheduledPosts =
                        "Disconnect Facebook Page and Delete Scheduled Posts";
                } else if (pageType == 'pinterest') {
                    confirmationText =
                        "Are you sure you want to delete this Pinterest board?";
                    confirmationTextInside =
                        "Disconnect This Pinterest Board Only |or| Also Delete It's Scheduled Posts?";
                    disconnectOnly = "Disconnect Pinterest Board Only";
                    disconnectDeleteScheduledPosts =
                        "Disconnect Pinterest Board and Delete Scheduled Posts";
                } else if (pageType == 'instagram') {
                    confirmationText =
                        "Are you sure you want to delete this Instagram account?";
                    confirmationTextInside =
                        "Disconnect Instagram Only |or| Also Delete It's Scheduled Posts?";
                    disconnectOnly = "Disconnect Instagram Only";
                    disconnectDeleteScheduledPosts =
                        "Disconnect Instagram and Delete Scheduled Posts";
                } else if (pageType == 'fbgroup') {
                    confirmationText =
                        "Are you sure you want to delete this Facebook group?";
                    confirmationTextInside =
                        "Disconnect This Facebook Group Only |or| Also Delete It's Scheduled Posts?";
                    disconnectOnly = "Disconnect Facebook Group Only";
                    disconnectDeleteScheduledPosts =
                        "Disconnect Facebook Group and Delete Scheduled Posts";
                } else if (pageType == 'youtube') {
                    confirmationText =
                        "Are you sure you want to delete this YouTube channel?";
                    confirmationTextInside =
                        "Disconnect This youTube Group Only |or| Also Delete It's Scheduled Posts?";
                    disconnectOnly = "Disconnect YouTube channel Only";
                    disconnectDeleteScheduledPosts =
                        "Disconnect YouTube channel and Delete Scheduled Posts";
                }
                ...
                swal({
                    ...
                    if (isConfirmed) {
                        // User confirmed, handle the delete action
                        swal({
                            title: "Select Your Action",
                            text: confirmationTextInside,
                            type: "warning",
                            showCancelButton: true,
                            confirmButtonColor: "#DD6B55",
                            cancelButtonText: disconnectOnly,
                            confirmButtonText: disconnectDeleteScheduledPosts,
                            closeOnConfirm: false
                        }, function(innerIsConfirmed) {
                            // Set the action based on the inner confirmation
                            var action = innerIsConfirmed ?
                                "disconnect_and_delete" :
                                "disconnect_only";

                            handleDelete(id, pageType,
                                button, action);
                        });
                    }
                });
            ...
            // Mouse leave, hide the delete button by setting opacity to 0
