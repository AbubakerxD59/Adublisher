...
class Users
{
	...
	const fbTable = "facebook_pages";
	const pinTable = "pinterest_boards";
	const pinUserTable = "pinterest_users";
	const ytTable = "youtube_channels";
	...
	public function Index()
	{
		...
		if (isset($_GET['letter']) and $find) {
			...
			$counter = self::$db->count(false, false, "SELECT COUNT(*) FROM `" . self::mTable . "` $where AND `fname` LIKE '%" . trim($find) . "%' OR `lname` LIKE '%" . trim($find) . "%'  OR `email` LIKE '%" . trim($find) . "%'");
	...
	public function Active()
	{
		...
		$activeUsers = "SELECT DISTINCT user_id
			FROM (
				SELECT user_id FROM rsssceduler WHERE 'created_at' >= $startDate OR 'created_at' <= $endDate
				UNION
				SELECT user_id FROM pinterest_scheduler WHERE 'created_at' >= $startDate OR 'created_at' <= $endDate
				UNION
				SELECT user_id FROM youtube_scheduler WHERE 'created_at' >= $startDate OR 'created_at' <= $endDate
			) AS combined_user_ids;";

		$activeUserArray = self::$db->pdoQuery($activeUsers)->results();
		$user_ids = [];
		foreach($activeUserArray as $key => $value){
	...
	public function Edit($id)
	{
		...
		if (!$row = Db::run()->first(self::mTable, null, array("id =" => $id, "AND type <>" => "owner"))) {
			...
			$tpl->data = $row;
			//   facebook pages
			$facebookSql = "SELECT * from `" . self::fbTable . "` where user_id = ?";
			$tpl->facebook_count = self::$db->pdoQuery($facebookSql, array($id))->results();
			// pinterest boards
			$pinterestSql = "SELECT * from `" . self::pinTable . "` where user_id = ?";
			$tpl->pinterest_count = self::$db->pdoQuery($pinterestSql, array($id))->results();
			$pinterestUserSql = "SELECT * from `" . self::pinUserTable . "` where user_id = ?";
			$tpl->pinterestUser = self::$db->pdoQuery($pinterestUserSql, array($id))->results();
			//   youtube channels
			$youtubeSql = "SELECT * from `" . self::ytTable . "` where user_id = ?";
			$tpl->youtube_count = self::$db->pdoQuery($youtubeSql, array($id))->results();
	...
	public function _addUser($rules, $filters)
	{
		...
		if (empty(Message::$msgs)) {
			...
			if (Validator::post('add_trans')) {
				...
				// insert membership features and limits
				$features = Membership::getPackageFeatures($safe->membership_id);
				foreach($features as $key => $feature){
					$fdata = [
						'pid' => $safe->membership_id,
						'fid' => $feature->id,
						'uid' => $last_id,
						'quota' => $feature->feature_limit,
						'used' => 0,
						'is_unlimited' => $feature->is_unlimited
					];
					Db::run()->insert(Membership::pfTable, $fdata);	
				}
			}
	...
	public function _updateUser($rules, $filters)
	{
		...
		$safe = $validate->doValidate($_POST, $rules);
		$safe = $validate->doFilter($_POST, $filters);
		$user = Db::run()->first(self::mTable, null, array("id =" => $safe->id, "AND type <>" => "owner"));
		...
		if (empty(Message::$msgs)) {
			...
			if ($user->membership_id != $safe->membership_id) {
				if ($_POST['membership_id'] > 0) {
					// update user expiry date and membership id
					$data['mem_expire'] = Membership::calculateDays($safe->membership_id);
					$data['membership_id'] = $safe->membership_id;
					// get membership from table
					$row = Db::run()->first(Membership::mTable, null, array("id" => $safe->membership_id, "private" => 0));
					// update user resources
					Membership::updateResources($safe->membership_id, $user->id);
				} else {
					$data['membership_id'] = 0;
				}
			}
			if ($user->mem_expire != $safe->mem_expire_submit) {
				$mem_expire_submit = str_replace('-', '/', $safe->mem_expire_submit);
				$data['mem_expire'] = date('Y-m-d H:i:s', strtotime($mem_expire_submit));
			}
