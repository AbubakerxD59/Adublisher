<?php
  ...
  class File
  {
      ...
      {
          $result = false;
          $dirPath = (($fullPath) ? BASEPATHMM : '') . $src;
          ...
          if (is_dir($dirPath)) {
              ...
              if (!file_exists(trim($dest, '/') . '/'))
                  mkdir((($fullPath) ? BASEPATHMM : '') . $dest);
          ...
          if (!file_exists($directory) || !is_dir($directory)) {
              ...
              foreach ($all_files as $file) {
                  ...
                  if ($file->isDir()) {
                      $dirs[] = array(
                          'path' => $real_path,
                          'url' => str_replace(BASEPATHMM, "", $file->getPathname()) . "/",
                          'name' => str_replace("_", " ", $file->getBasename()),
                          'kind' => 'directory',
                          'total' => iterator_count(new FilesystemIterator($file->getPathname(), FilesystemIterator::SKIP_DOTS)));
                  }
       ...
       * File::copyFile()
       * 
       * @param string $src (absolute path BASEPATHMM . $sourcePath)
       * @param string $dest (absolute path BASEPATHMM . $targetPath)
       * @param string $src
       * @param string $dest
       * @return
       */
