...
$filter = Validator::get('filter') ? Validator::get('filter') : null;
/* == GET Actions == */
switch ($gAction):
	/* == Edit Role == */
	case "editRole":
		$tpl = App::View(BASEPATHMM . 'view/admin/snippets/');
	...
	case "languageSection":
		$xmlel = simplexml_load_file(BASEPATHMM . Lang::langdir . Core::$language . ".lang.xml");
	...
	case "exportUsers":
		...
		header('Content-Type: text/csv; charset=utf-8');
		if ($filter == 'active') {
			header('Content-Disposition: attachment; filename=ActiveUserList.csv');
			$result = Stats::exportActiveUsers();
		} 
		elseif ($filter == 'inactive') {
			header('Content-Disposition: attachment; filename=InactiveUserList.csv');
			$result = Stats::exportInactiveUsers();
		}
		else {
...
switch ($pAction):
	...
	case "editPhrase":
		if (file_exists(BASEPATHMM . Lang::langdir . Core::$language . ".lang.xml")):
			$xmlel = simplexml_load_file(BASEPATHMM . Lang::langdir . Core::$language . ".lang.xml");
			$node = $xmlel->xpath("/language/phrase[@data = '" . $_POST['key'] . "']");
			$node[0][0] = $title;
			$xmlel->asXML(BASEPATHMM . Lang::langdir . Core::$language . ".lang.xml");
