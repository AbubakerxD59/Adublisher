...
<div class="wojo fitted segment">
  <table class="wojo responsive table" id="backupList">
    <?php if ($this->data):?>
    <?php foreach ($this->data as $i => $row):?>
    <?php $i++;?>
    <?php $latest =  ($row == App::Core()->backup) ? " highlite" : null;?>
    <tr class="<?php echo $latest;?>">
      <td><span class="wojo small bold text"><?php echo $i;?>.</span>
        <?php echo str_replace(".sql", "", $row);?></td>
      <td class="auto"><span class="wojo small dark inverted label"><?php echo File::getFileSize($this->dbdir . '/' . $row, "kb", true);?></span></td>
      <td class="auto"><a href="<?php echo UPLOADURL . '/backups/' . $row;?>" data-content="<?php echo Lang::$word->DOWNLOAD;?>" class="wojo icon positive inverted circular button button"><i class="download icon"></i></a>
        ...
        <a data-set='{"option":[{"delete": "deleteBackup","title": "<?php echo $row;?>","id":1}],"action":"delete","parent":".item"}' class="wojo icon negative inverted circular button data"><i class="icon trash"></i></a></td>
    </tr>
    <?php endforeach;?>
    <?php unset($row);?>
    <?php endif;?>
