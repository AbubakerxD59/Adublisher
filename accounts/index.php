...
$router->mount('/admin', function () use ($router, $tpl) {
	...
	$router->mount('/users', function () use ($router, $tpl) {
		...
		$router->match('GET|POST', '/active', 'Users@Active');
		$router->match('GET|POST', '/inactive', 'Users@Inactive');
		$router->match('GET|POST', '/grid', 'Users@Index');
		$router->get('/history/(\d+)', 'Users@History');
		$router->get('/edit/(\d+)', 'Users@Edit');
		$router->get('/active/edit/(\d+)', 'Users@Edit');
		$router->get('/inactive/edit/(\d+)', 'Users@Edit');
	...
	//admin blogs
	$router->mount('/blogs', function () use ($router, $tpl) {
		$router->match('GET', '/', 'Blog@Index');
		$router->get('/new', 'Blog@Save');
		$router->get('/edit/(\d+)', 'Blog@Edit');
		$router->match('POST', '/image_upload', 'Blog@imageUpload');
	});
	//admin categories
	$router->mount('/categories', function () use ($router, $tpl) {
		$router->match('GET', '/', 'Category@Index');
		$router->get('/new', 'Category@Save');
		$router->get('/edit/(\d+)', 'Category@Edit');
	});
	//admin system
