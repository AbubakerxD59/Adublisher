...
class Facebook
{
        ...
        try {
            if(isset($params) && is_array($params)){
                foreach ($params as $key => $param) {
                    if (in_array($key, ['message', 'caption', 'description'])) {
    ...
    public function addpage_url()
    {
        ...
        return $this->helper->getLoginUrl(
            $this->config->item('facebook_login_redirect_url_addpage'),
            $this->config->item('facebook_permissions')
        );
    }
    public function user_setDefaultAccessToken($access_token)
    {
        $dateTimeObj = new DateTime();
        $dateTimeObj->add(new DateInterval("PT1H"));
        App::Session()->set('fb_access_token', $access_token);
        $this->set_expire_time($dateTimeObj);
    ...
    {
        return App::Session()->get('fb_access_token');
    }
    ...
    {
        App::Session()->set('fb_access_token', $access_token->getValue());
    }
    ...
    {
        return App::Session()->get('fb_expire');
    }
    ...
    {
        if ($time) {
            App::Session()->set('fb_expire', $time->getTimestamp());
        }
    }
